...
自定义直连 / 代理规则和专用业务规则优先，随后是 GFW 通用代理规则，最后默认直连。GitHub 规则独立于微软服务规则，并排在微软规则之前；Netflix 保持独立策略组。Telegram 同时加载域名和官方 IP 段规则，以覆盖移动端直连 IP 的流量。已移除上游失效的 `ProxyMedia` / `🌍 国外媒体` 规则集与策略组，避免规则下载 404。
